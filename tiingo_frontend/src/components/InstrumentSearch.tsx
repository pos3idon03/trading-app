...
function formatLabel(instrument: Instrument): string {
  const parts = [instrument.symbol, instrument.name ?? instrument.symbol];
  if (instrument.exchange) parts.push(`(${instrument.exchange})`);
  return parts.join(' — ');
}
...
export default function InstrumentSearch({
  ...
  onSelect,
  assetTypeFilter,
  placeholder = 'Search ingested instruments (symbol or name)',
  className = '',
  ...
  useEffect(() => {
    ...
    ingestionApi
      .searchDbInstruments(debouncedQuery, assetTypeFilter)
      .then((items) => {
        if (cancelled) return;
        setResults(items);
        setHasSearched(true);
  ...
  }, [debouncedQuery, assetTypeFilter, selected]);
  ...
  const pickResult = (instrument: Instrument) => {
    onSelect(instrument);
    setQuery(formatLabel(instrument));
    setResults([]);
    setOpen(false);
    setActiveIndex(-1);
  };
    ...
    <div ref={containerRef} className={`relative min-w-[240px] flex-1 ${className}`}>
        ...
        >
          {results.map((instrument, idx) => (
            ...
            >
              ...
              <span className="truncate flex-1">{instrument.name ?? instrument.symbol}</span>
              <span className="text-xs text-slate-500 shrink-0 capitalize">{instrument.asset_type}</span>
            </li>
          ))}
        </ul>
      )}
      ...
      {open && !selected && !loading && hasSearched && results.length === 0 && (
        <div className="absolute z-50 mt-1 w-full bg-surface-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-500">
          {searchError ?? 'No ingested instruments match. Add symbols via Ingestion → Watchlist.'}
        </div>
      )}
    </div>
  );
